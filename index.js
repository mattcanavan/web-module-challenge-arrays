...
is31Flavors(originalFlavors)    //output: True
...
function addFlavor(iceCreamArray, newFlavor){

    iceCreamArray.unshift(newFlavor);
...
function removeLastFlavor(iceCreamArray){
...
function getFlavorByIndex(iceCreamArray, indexValue){

    console.log(iceCreamArray[indexValue]);
...
function removeFlavorByName(iceCreamArray, flavorName){

    for (i=0; i < iceCreamArray.length; i++){
        if (iceCreamArray[i] === flavorName){
            iceCreamArray.splice(i);
        }
    }
}
...
var originalFlavorsCopy1 = [];

function copy(originalArray, originalArrayCopy){

    originalArrayCopy = originalArray.slice(0);
    return originalArrayCopy

}

console.log(copy(originalFlavors, originalFlavorsCopy1));
...
function filterByWord(arrayIn, iceCreamDescription){

    let newArray = [];

    for (i=0; i < arrayIn.length; i++){
        if (arrayIn[i].includes(iceCreamDescription)){
